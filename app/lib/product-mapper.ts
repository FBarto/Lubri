...
export function mapLegacyProductCode(legacyDescription: string, category?: string): string | null {
    ...
    if (clean.includes('Q7000') || clean.includes('TOTAL 7000')) return 'TOTAL-Q7000-1L';
    if (clean.includes('Q5000') || clean.includes('TOTAL 5000')) return 'TOTAL-Q5000-1L';
    ...
    if (isPureNumber) {
        if (category === 'OIL_FILTER') return `MAP-${clean}`;
        if (category === 'AIR_FILTER') return `AMPI-${clean}`;
        if (category === 'CABIN_FILTER') return `HM-${clean}`;
    }

    // 3. Fuel Filter Transformation (f10230 -> G10230)
    ...
    if (clean.includes('AMPI') || clean.includes('AMP ')) {
        const num = clean.replace(/\D/g, '');
        if (num) return `AMPI-${num}`;
    }
